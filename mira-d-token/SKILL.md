---
name: "mira-d-token"
description: "Mira ��ϵ�е� mira-d-token Skill"
---
...
## 输入
- `style_keywords`: 风格关键词列表（�?`["科技�?, "暗色"]`�?- `theme_preference`: 主题偏好（`light` / `dark` / `both`�?
## 设计令牌结构定义
```json
{
...
## 输出格式
直接输出完整�?design-tokens.json 文件内容，并附带简要的翻译说明�?
## 约束
- �?theme_preference �?both，需同时生成 light �?dark 两套颜色的完整定义�?- 深色主题的颜色值须满足 WCAG 对比�?�?4.5:1（可参�?@wcag/color 库）�?- 字体优先级：用户指定 > 系统字体�?> 无衬线体�?