---
name: "mira-b-contract"
description: "Mira ��ϵ�е� mira-b-contract Skill"
---
...
## 输入
- `data_fields`: 数据字段列表（来�?`mira-o-vision`�?- `domain_rules`: 领域规则（来�?`mira-b-domain`�?
## 输出文件结构
...
## 约束
- 所有类型名必须�?I 开头（Interface 惯例），或使�?type 关键字但加上 T 前缀（如 TTodo）�?- 日期字段统一使用 string（ISO 8601 格式），后端返回时序列化，前端展示时使用 new Date() 解析�?- 分页参数统一使用 page �?limit 字段名，便于跨模块复用�?