...
## 描述
整合所有输入（Core 层、UI 组件、设计令牌、GWT 用例），为每个目标平台生成**完整的、可编译的源码文件**。这是 Mira-B 的"总装车间"。

## 输入
- `core_files`: 来自 `mira-b-core` 的 Core 层文件列表
- `ui_components`: 来自 `mira-b-ui` 的 UI 组件代码
- `design_tokens`: 来自 `mira-d-token` 的设计令牌
- `api_contracts`: 来自 `mira-b-contract` 的 API 契约
- `platforms`: 目标平台列表
- `gwt_cases`: 来自 `mira-b-gwt` 的测试用例
...
| mira-d-token | web/tailwind.config.js + globals.css | 转换为 Tailwind 主题扩展 + CSS 变量 |
| mira-b-ui | web/components/ | 按平台适配器要求组装 |
...
## 约束
- 每个平台独立生成完整的代码树，互不干扰。
- 编译检查（tsc --noEmit）必须通过，否则阻塞后续流程。
- 若某个平台的代码生成失败，不影响其他平台，但需在输出中明确标注失败平台及原因。
