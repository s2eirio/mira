...
## 描述
生成平台无关的 Core 层代码，包含状态管理（Stores）、业务服务（Services）、类型定义（Types）、工具函数（Utils）。Core 层**禁止引入任何平台特定 API**（如 `window`、`wx`、`localStorage`），所有平台依赖必须通过抽象接口注入。

## 输入
- `entities`: 需求实体（来自 `mira-o-vision`）
- `domain_rules`: 领域规则（来自 `mira-b-domain`）
- `api_contracts`: API 契约（来自 `mira-b-contract`，若已生成）
...
frontend/core/
├── stores/
│   └── todo.store.ts              # Zustand 状态管理
├── services/
│   ├── http.client.ts             # 抽象 HTTP 客户端（接口定义，无具体实现）
│   └── todo.service.ts            # 业务服务（调用 HTTP 客户端）
├── types/
│   └── todo.types.ts              # TypeScript 类型定义
└── utils/
    ├── formatter.util.ts          # 数据格式化工具
    └── validator.util.ts          # 数据校验工具
```

## Core 层代码示例

### 1. 类型定义（types/todo.types.ts）
```typescript
...
### 3. HTTP 客户端抽象（services/http.client.ts）
```typescript
// 平台无关的 HTTP 客户端接口
...
// 具体实现由各平台适配器注入
let _httpClient: IHttpClient | null = null;
...
### 4. 业务服务（services/todo.service.ts）
```typescript
...
## 约束
- Core 层代码必须 100% 无平台特定 API，所有平台依赖（如 fetch、localStorage、window）必须通过接口抽象注入。
- 状态管理优先使用 Zustand（因其轻量且支持 React Native），若用户偏好其他方案可调整。
- 所有服务方法的返回值必须使用 ITodo 等类型定义，禁止使用 any。
- 若领域规则包含审计日志，需在 Service 层自动添加日志调用。
