...
## 描述
将 `design-tokens.json` 中的设计变量，映射为各平台（Web / 小程序 / RN / Electron）的样式代码，确保多端视觉一致性。

## 输入
- `design_tokens`: 来自 `mira-d-token` 的完整 JSON
- `platforms`: 目标平台列表（来自 `mira-p-sniffer`）

## 样式映射规则表
| 设计令牌 | Web (Tailwind) | 小程序 (rpx) | React Native (StyleSheet) |
| :--- | :--- | :--- | :--- |
...
## 约束
- 必须为每个目标平台输出完整的样式代码。
- 小程序 rpx 转换规则：1px = 2rpx（以 375px 设计稿为基准）。
- React Native 尺寸使用 逻辑像素（pt），与 Web 的 px 值保持一致。
- 若平台列表只包含 1 个，只输出对应平台的样式映射。
