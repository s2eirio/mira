...
2. **实体抽取**：应用名称、目标用户、核心功能列表、数据字段、性能要求、设计风格关键词。
3. **平台推断**：扫描文本中的平台关键词（WEB / MINIAPP / PC / APP）。
4. **约束提取**：时间约束、技术约束、预算约束。
5. **歧义检测**：标记歧义点，生成澄清问题列表。
...
```json
{
  ...
  "entities": {
    "app_name": "TaskFlow",
    "target_users": ["项目经理", "开发人员"],
    "core_features": ["创建任务", "分配负责人", "截止日期提醒"],
...
## 约束
- 若 confidence_score < 0.7，须附加"需要用户补充以下信息..."的提示。
- 无法识别的实体字段值设为 null。
