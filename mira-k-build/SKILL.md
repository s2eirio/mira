---
name: "mira-k-build"
description: "Mira ��ϵ�е� mira-k-build Skill"
---
...
## 输入
- `platforms`: 目标平台列表
- `version`: 当前版本号（用于产物命名�?- `output_dir`: 输出目录（默�?`./build/`�?
## 各平台构建命�?| 平台 | 构建命令 | 产物路径 |
| :--- | :--- | :--- |
...
## 约束
- 构建时使�?--mode production 或等效的生产环境变量�?- 构建前自动执�?pnpm install --frozen-lockfile，确保依赖版本一致�?- 若某个平台构建失败，不阻塞其他平台，但需在输出中明确标注失败平台及错误日志�?